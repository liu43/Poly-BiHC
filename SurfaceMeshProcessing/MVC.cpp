#include "MVC.h"
namespace MVC {
	static std::vector<Point2D> y;
	static std::vector<Point2D> z;
	static std::vector<double> vCoeff[3];
	static std::vector<double> gnCoeff[3];
	static std::vector<double> gtCoeff[3];
	...
	bool checkPolygon(const std::vector<Point2D>& poly) {
		if (int(poly.size()) < 3) {
			return false;
		}
	...
	void boundaryCoords(const std::vector<Point2D>& poly, const Point2D& p, std::vector<double>& vCoords, std::vector<double>& gnCoords, std::vector<double>& gtCoords, int i, int j) {
	...
	void MVCs(Eigen::MatrixXd& V, Eigen::MatrixXd& cage, Eigen::MatrixXd& weight) {
	...
	void MVCs(const std::vector<OpenMesh::Vec3d>& poly, const OpenMesh::Vec3d& p, std::vector<double>& vCoords)
	{
		Eigen::MatrixXd V(1, 2);//1*2的矩阵，放的p的x,y分量
		Eigen::MatrixXd cage(poly.size(), 2);//poly.size()*2的矩阵，放的poly的x,y分量
		Eigen::MatrixXd weight;//poly.size()*1的矩阵，值赋给vCoordinates
		V << p[0], p[1];  // 存储点的x,y分量
		for (size_t i = 0; i < poly.size(); ++i) {
			cage.row(i) << poly[i][0], poly[i][1];  // 存储多边形顶点x,y分量
		}
		MVCs(V, cage, weight);
		vCoords.clear();
		vCoords.reserve(weight.rows());
		for (int i = 0; i < weight.rows(); ++i) {
			vCoords.push_back(weight(i, 0));  // 提取权重到输出参数
		}
	}
	...
	void cubicMVCs(const std::vector<OpenMesh::Vec3d>& poly, const OpenMesh::Vec3d& p, std::vector<double>& vCoords, std::vector<double>& gnCoords, std::vector<double>& gtCoords) {
		// Convert OpenMesh::Vec3d to Point2D
		std::vector<Point2D> poly2D;
		for (const auto& vertex : poly) {
			poly2D.emplace_back(vertex[0], vertex[1]);
		}

		// Convert point p
		Point2D p2D(p[0], p[1]);

		// Call the existing cubicMVCs function
		cubicMVCs(poly2D, p2D, vCoords, gnCoords, gtCoords);
	}
}