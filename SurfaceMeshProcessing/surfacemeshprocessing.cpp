...
void SurfaceMeshProcessing::CreateActions(void)
{
	...
	actHiddenLines = new QAction(tr("curve Biharmonic"), this);
	...
	//将FlatLine转化为CubicMVC test.
	actFlatLines = new QAction(tr("straight_Bih"), this);
	...
	connect(actFlatLines, SIGNAL(triggered()), viewer, SLOT(ShowCubicMVC()));
	//将Flat转化为polyGC test.
	actFlat = new QAction(tr("PolyGC"), this);
	actFlat->setIcon(QIcon(":/SurfaceMeshProcessing/Images/flat.png"));
	actFlat->setStatusTip(tr("Show flat"));
	actFlat->setCheckable(true);
	connect(actFlat, SIGNAL(triggered()), viewer, SLOT(ShowPolyGC()));
	//将smooth转化为绘制curved cage
	actSmooth = new QAction(tr("Curve Cage"), this);
	actSmooth->setIcon(QIcon(":/SurfaceMeshProcessing/Images/smooth.png"));
	actSmooth->setStatusTip(tr("Show CurveCage"));
	actSmooth->setCheckable(true);
	connect(actSmooth, SIGNAL(triggered()), viewer, SLOT(ShowCurveCage()));

	//将smooth转化为绘制curved cage
	actVideo1 = new QAction(tr("Video1"), this);
	actVideo1->setIcon(QIcon(":/SurfaceMeshProcessing/Images/smooth.png"));
	actVideo1->setStatusTip(tr("Show Video1"));
	actVideo1->setCheckable(true);
	connect(actVideo1, SIGNAL(triggered()), viewer, SLOT(DrawVideo1()));

	//将smooth转化为绘制curved cage
	actVideo2 = new QAction(tr("Video2"), this);
	actVideo2->setIcon(QIcon(":/SurfaceMeshProcessing/Images/smooth.png"));
	actVideo2->setStatusTip(tr("Show Video2"));
	actVideo2->setCheckable(true);
	connect(actVideo2, SIGNAL(triggered()), viewer, SLOT(DrawVideo2()));

	//将smooth转化为绘制curved cage
	actVideo3 = new QAction(tr("Video3"), this);
	actVideo3->setIcon(QIcon(":/SurfaceMeshProcessing/Images/smooth.png"));
	actVideo3->setStatusTip(tr("Show Video3"));
	actVideo3->setCheckable(true);
	connect(actVideo3, SIGNAL(triggered()), viewer, SLOT(DrawVideo3()));
	...
	agViewGroup->addAction(actVideo1);
	agViewGroup->addAction(actVideo2);
	agViewGroup->addAction(actVideo3);
...
void SurfaceMeshProcessing::CreateMenus(void)
{
	...
	menuRenderMode->addAction(actVideo1);
	menuRenderMode->addAction(actVideo2);
	menuRenderMode->addAction(actVideo3);
...
void SurfaceMeshProcessing::CreateToolBars(void)
{
	...
	tbView->addAction(actVideo1);
	tbView->addAction(actVideo2);
	tbView->addAction(actVideo3);
